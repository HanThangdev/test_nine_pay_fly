...
import {
  DataFetchFile,
  DataFetchLink,
  GetAdvanceSettingPayload,
  HistoryChat,
} from './type';
import {
  addQuestionAndAnswerTransaction,
  createSessionTransaction,
  deleteFileImportedTransaction,
...
export const useBuildChatbot = () => {
  ...
  } = useSelector((state: RootState) => state.buildChatBot, shallowEqual);
  ...
  const onStreamingUploadUrl = useCallback(
    async (payload: ImportURLPayload) => {
      ...
      const response = await dispatch(
        importURLTransaction({ payload, callBack }),
      );
      return response;
    },
    [dispatch],
